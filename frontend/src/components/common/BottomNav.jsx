        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center w-20 h-full transition-colors group ${
                isActive ? 'text-on-primary-container' : 'text-on-surface-variant hover:text-primary'
              }`}
            >
              {isActive ? (
                <div className="bg-primary-container rounded-full px-5 py-1 mb-1">
                  <span className="material-symbols-outlined text-2xl text-on-primary-container filled">
                    {tab.icon}
                  </span>
                </div>
              ) : (
                <span className="material-symbols-outlined text-2xl mb-1 group-hover:bg-surface-container-highest rounded-full px-4 py-1 transition-all duration-300">
                  {tab.icon}
                </span>
              )}
